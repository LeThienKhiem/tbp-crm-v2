...
export default function MarketRadarPage() {
  ...
  const filteredRates = useMemo(() => {
    if (regionFilter === "All") return mockRates;
            ...
            {filteredRates.map((rate) => {
              ...
              return (
                <div key={rate.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <Link
                    href={`/market-radar/quote/${rate.id}`}
                ...
                {filteredRates.map((rate) => {
                  ...
                  return (
                    <tr key={rate.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-right font-medium text-orange-600">{formatCurrency(rate.pss)}</td>
